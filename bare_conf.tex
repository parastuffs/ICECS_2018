...
\title{3D-Stacked Integrated Circuits: \\ How Fine System Partitioning Should Be?}
...
% in the abstract
\begin{abstract}
3D stacked ICs package multiple, independently manufactured dies to reduce total system wire-length and improve timing, reduce area and power. When designing stacked 3D-ICs, arises the question of the grain at which one should consider system partitioning to optimize the gains. In this work we use known graph partitioning algorithm to split designs from 42k gates and up to 800k gates, with gates clustered in 8 and up to 32768 partitions. We find that with 2048 clusters, 20 to 400 gates per cluster depending on the design, a partitioning of the system allows on average to cut 35\% of the nets that account for 73\% of the total wire-length in 3D.
\end{abstract}
...
In our future work, we will explore alternative clustering methods in order to further reduce the amount of short nets in the partitioning cut and further optimize the partitioning decision, including multi-criteria optimization methods.
