...
% in the abstract
\begin{abstract}
3D stacked ICs package multiple, independently manufactured dies to reduce total system wire-length, improve timing, and reduce area and power. When designing stacked 3D-ICs, arises the question of the grain at which one should consider system partitioning to optimize the gains. In this work we use known graph partitioning algorithms to split designs from 42k gates and up to 800k gates, with gates clustered in 8 and up to 32768 partitions. We find that with 2048 clusters, i.e. 20 to 400 gates per cluster depending on the design, a partitioning of the system allows on average to cut 35\% of the nets that account for 73\% of the total wire-length in 3D.
\end{abstract}
